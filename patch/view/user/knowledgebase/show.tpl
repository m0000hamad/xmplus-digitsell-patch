         ...
         <div class="d-grid gap-3 gap-lg-5">
            <div class="card shadow-lg rounded">
                <div class="card-header card-header-content-between">
                    <h4 class="card-header-title">{$knw->title}</h4>
                </div>
                <div class="card-body">
					{$content = json_decode($knw->content,true)}
					{assign var=lang value="_"|explode:$session->get('locale')}
					<p class="text-dark fs-5 mb-1">{if isset($content[$lang[1]])}{str_replace(['%SubUrl%'],[$SubUrl], $content[$lang[1]])}{/if}</p>
                </div>
			</div>  
		</div>	  	  
	</div>
...
<script> 
    ...
    $('.shadowrocket').click(function(e) {
		Swal.fire({
			...
			if (result.isConfirmed) {
				url = "{$SubUrl}?shadowrocket=1"
				let tmp = window.btoa(url);
				window.location.href = "shadowrocket://add/sub://" + tmp;
			}
		})
    })

	$('.surfboard').click(function(e) {
		url = "{$SubUrl}?surfboard=1"
		window.location.href = 'surfboard:///install-config?url=' + encodeURIComponent(url);
    })	
