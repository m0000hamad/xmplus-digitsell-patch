// Compiles Tailwind for the login page and inlines it at /*TAILWIND*/.
...
execSync(`npx --yes tailwindcss@3.4.17 -c tools/login/tailwind.config.js -i tools/login/.in.css -o ${JSON.stringify(out)} --minify`, { cwd: root, stdio: 'inherit' });
...
const src = fs.readFileSync(path.join(__dirname, 'login.src.tpl'), 'utf8');
fs.writeFileSync(path.join(root, 'patch/view/auth/login.tpl'), src.replace('/*TAILWIND*/', css));
fs.unlinkSync(out); fs.unlinkSync(path.join(__dirname, '.in.css'));
console.log('login.tpl written,', css.length, 'bytes of CSS');
